...
import static com.BSISJ7.TestCreator.testIO.XmlUtil.asList;
...
public class Test {
    ...
    public void loadFromXMLNode(Element testNode) {
        testName = findNode("TestName", testNode).getTextContent();
        description = findNode("TestDescription", testNode).getTextContent();
        for (Node questionNode : asList(testNode.getElementsByTagName("Question"))) {
            try {
                String questionName = findNode("QuestionName", questionNode).getTextContent();
                String questionType = findNode("QuestionType", questionNode).getTextContent();
